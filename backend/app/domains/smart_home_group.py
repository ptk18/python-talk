...
def _parse_colour(raw: str) -> str:
    raw_lower = raw.strip().lower()
    if re.match(r'^#[0-9a-fA-F]{6}$', raw.strip()):
        return raw.strip()
    for name, hex_code in COLOUR_MAP.items():
...
class Lightbulb:
    def __init__(self, uuid, name, home):
        self.uuid = uuid
        self.name = name
        self.home = home

    def turn_on(self):
        self.home._ensure_authenticated()
        url = f"{self.home.base_url}/homes/lightbulbs/{self.uuid}/"
        headers = {"Authorization": f"Token {self.home.token}"}
        response = requests.patch(url, json={"is_on": True}, headers=headers, verify=False, timeout=10)
        print(f"[TURN_ON_{self.name}] Status: {response.status_code}, Response: {response.text}")
        if response.status_code == 200:
            print(f"{self.name} turned on")
        else:
            print(f"Failed to turn on {self.name}: {response.text}")
    ...
    def turn_off(self):
        ...
        print(f"[TURN_OFF_{self.name}] Status: {response.status_code}, Response: {response.text}")
        if response.status_code == 200:
            print(f"{self.name} turned off")
        else:
            print(f"Failed to turn off {self.name}: {response.text}")

    def set_colour(self, colour):
        colour = _parse_colour(colour)
        self.home._ensure_authenticated()
        url = f"{self.home.base_url}/homes/lightbulbs/{self.uuid}/set_colour/"
        headers = {"Authorization": f"Token {self.home.token}"}
        response = requests.post(url, json={"colour": colour}, headers=headers, verify=False, timeout=10)
        print(f"[SET_COLOUR_{self.name}] Status: {response.status_code}, Response: {response.text}")
        if response.status_code == 200:
            print(f"{self.name} colour changed to {colour}")
        else:
            print(f"Failed to change {self.name} colour: {response.text}")
...
class AC:
    ...
    def turn_on(self):
        self.home._ensure_authenticated()
        url = f"{self.home.base_url}/homes/acs/{self.uuid}/"
        headers = {"Authorization": f"Token {self.home.token}"}
        response = requests.patch(url, json={"is_on": True}, headers=headers, verify=False, timeout=10)
...
class Fan:
    ...
    def turn_on(self):
        self.home._ensure_authenticated()
        url = f"{self.home.base_url}/homes/fans/{self.uuid}/"
        headers = {"Authorization": f"Token {self.home.token}"}
        response = requests.patch(url, json={"is_on": True}, headers=headers, verify=False, timeout=10)
...
class TV:
    ...
    def turn_on(self):
        self.home._ensure_authenticated()
        url = f"{self.home.base_url}/homes/tvs/{self.uuid}/"
        headers = {"Authorization": f"Token {self.home.token}"}
        response = requests.patch(url, json={"is_on": True}, headers=headers, verify=False, timeout=10)
...
class SmartHome:
    DEVICE_ALIASES = {
        ...
        "ac": "AC", "air conditioner": "AC", "air con": "AC",
        "fan": "Fan",
        "tv": "TV", "television": "TV",
    }
    ...
    def __init__(self):
        ...
        self.lightbulb1 = Lightbulb("f5398b01-79c5-4f88-b9ea-019b452d6de6", "Lightbulb 1", self)
        self.lightbulb2 = Lightbulb("badec2bd-10e0-40ff-a38d-95b8d80c4504", "Lightbulb 2", self)
        self.ac = AC("35da478b-b03b-4509-9f66-979c76bfa3d9", "AC", self)
        self.fan = Fan("eadcc85f-71c5-4452-b3ac-f2276a5cfa7a", "Fan", self)
        self.tv = TV("f914aefe-76d2-44fd-bc0a-bf536f0bd227", "TV", self)

        self.add(self.lightbulb1)
        self.add(self.lightbulb2)
        self.add(self.ac)
        self.add(self.fan)
        self.add(self.tv)

    def add(self, device):
        """Add a device to this smart home.

        Phrases: add device, register device, add to home, connect device.
        """
        self.devices[device.name] = device
        print(f"Added {device.name} to smart home")

    def _find_device(self, device):
        canonical = self.DEVICE_ALIASES.get(device.strip().lower())
        if canonical:
            return self.devices.get(canonical)
    ...
    def turn_on(self, device):
        ...
        turn on lightbulb 2, turn on light 2, switch on lightbulb 2, switch on light 2,
        ...
        turn on ac, switch on ac, turn ac on, switch ac on,
        enable ac, power on ac, start ac, ac on,
        turn on air conditioner, turn on air con,
        open ac, open air conditioner, open air con,
        turn on fan, switch on fan, turn fan on, switch fan on,
        enable fan, power on fan, start fan, fan on, open fan,
        turn on tv, switch on tv, turn tv on, switch tv on,
        enable tv, power on tv, start tv, tv on,
        turn on television, open tv, open television.
        """
        dev = self._find_device(device)
        if dev:
            dev.turn_on()
        else:
    ...
    def turn_off(self, device):
        ...
        turn off lightbulb 2, turn off light 2, switch off lightbulb 2, switch off light 2,
        ...
        turn off ac, switch off ac, turn ac off, switch ac off,
        ...
        turn off fan, switch off fan, turn fan off, switch fan off,
        disable fan, power off fan, stop fan, fan off, close fan,
        turn off tv, switch off tv, turn tv off, switch tv off,
        disable tv, power off tv, stop tv, tv off,
        turn off television, close tv, close television.
        """
        dev = self._find_device(device)
        if dev:
            dev.turn_off()
    ...
    def set_colour(self, device, colour):
        """Change a device colour.
        ...
        change lightbulb 2 color, change light 2 color,
        ...
        """
        dev = self._find_device(device)
        if dev and hasattr(dev, 'set_colour'):
            dev.set_colour(colour)
        else:
            print(f"Device '{device}' does not support set_colour")
