...
export const conversationAPI = {
  ...
  async getSingleConversation(conversationId) {
    return apiCall(`/conversations/${conversationId}/single`);
  },
};
...
export const googleSpeechAPI = {
  checkStatus: async () => {
    return apiCall('/google-speech/status');
  },
  ...
  textToSpeech: async (text) => {
    ...
    const response = await fetch(`${API_BASE_URL}/google-speech/text-to-speech?${params}`, {
      method: 'POST',
      headers: token ? { 'Authorization': `Bearer ${token}` } : {},
    });
  ...
  speechToText: async (audioFile, language = 'en') => {
    ...
    const token = getAuthToken();
    const response = await fetch(`${API_BASE_URL}/google-speech/speech-to-text`, {
      method: 'POST',
      headers: token ? { 'Authorization': `Bearer ${token}` } : {},
      body: formData,
    });
...
export const analyzeAPI = {
  ...
  prewarmPipeline: async (conversation_id) => {
    return apiCall(`/prewarm_pipeline`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  invalidatePipelineCache: async (conversation_id) => {
    return apiCall(`/invalidate_pipeline_cache`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
...
export const fileAPI = {
  listFiles: async (conversation_id) => {
    const params = new URLSearchParams({ conversation_id: conversation_id.toString() });
    return apiCall(`/list_files?${params}`, {
      method: "GET",
    });
  },
  ...
  getFile: async (conversation_id, filename) => {
    ...
    return apiCall(`/get_file?${params}`, {
      method: "GET",
    });
  },

  saveFile: async (conversation_id, filename, code) => {
    return apiCall(`/save_file`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
...
export const paraphraseAPI = {
  getParaphrases: async (text, maxVariants = 10) => {
    return apiCall(`/user_command_paraphrasing_suggestion`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
...
export const translateAPI = {
  ...
  checkStatus: async () => {
    return apiCall(`/translate/status`);
  },
};

export const turtleAPI = {
  analyzeCommand: async (command, language = 'en') => {
    return apiCall(`/analyze_turtle_command`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  getMethods: async () => {
    return apiCall(`/turtle_methods`, {
      method: 'GET',
    });
  },

  prewarmPipeline: async () => {
    return apiCall(`/prewarm_turtle_pipeline`, {
      method: 'POST',
    });
  },
};
...
export const favoritesAPI = {
  ...
  async toggle(userId, data) {
    return apiCall(`/favorites/${userId}/toggle`, {
      method: 'POST',
      body: JSON.stringify(data),
    });
  },
