<template>
  <div class="app-card" @click="selectApp">
    <div class="app-icon">
    ...
    <button class="install-btn" @click="openApp">Get</button>
  </div>
</template>
...
export default {
  ...
  methods: {
    selectApp() {
      console.log('Selected app:', this.app.name)
    }
  },
  openApp() {
    if (this.app.url) {
      window.open(this.app.url, '_blank') // or same tab: location.href = this.app.url
    } else {
      console.warn('No URL defined for app:', this.app.name)
    }
  }
}
</script>
